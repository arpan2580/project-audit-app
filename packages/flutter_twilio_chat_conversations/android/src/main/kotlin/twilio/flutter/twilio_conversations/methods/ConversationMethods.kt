...
class ConversationMethods : Api.ConversationApi {
    ...
    override fun sendMessage(
        ...
        // Conversations 6.x replaced Message.Options with a MessageBuilder that
        // is obtained from the Conversation itself, so the options can only be
        // assembled once the conversation has been fetched. Validate anything
        // that can fail synchronously before starting the async call, to keep
        // the previous error behaviour.
        val mediaInputPath = options.inputPath as String?
        val mediaMimeType = options.mimeType as String?
        if (mediaInputPath != null && mediaMimeType == null) {
            ...
            client.getConversation(conversationSid, object : CallbackListener<Conversation> {
                override fun onSuccess(conversation: Conversation) {
                    ...
                    if (options.body != null) {
                        messageBuilder.setBody(options.body as String)
                    }

                    if (options.attributes != null) {
                        val attributes = Mapper.pigeonToAttributes(options.attributes)
                            ?: return result.error(ConversionException("Could not convert ${options.attributes} to valid Attributes"))
                        messageBuilder.setAttributes(attributes)
                    }

                    // Local copies so Kotlin can smart-cast the captured
                    // nullable vals; both are non-null together, as validated
                    // before the call above.
                    val inputPath = mediaInputPath
                    val mimeType = mediaMimeType
                    if (inputPath != null && mimeType != null) {
                        // TODO: forward upload progress to mediaProgressSink once
                        // Api.MessageOptionsData.mediaProgressListenerId is wired up.
                        messageBuilder.addMedia(
                            ...
                            object : MediaUploadListener {
                                ...
                                override fun onFailed(errorInfo: ErrorInfo) {
                                    debug("sendMessage => media upload failed: $errorInfo")
                                }
                            }
                        )
                    }

                    messageBuilder.buildAndSend(object : CallbackListener<Message> {
