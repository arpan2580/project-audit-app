            ...
            override fun onSuccess(conversation: Conversation) {
                conversation.getMessageByIndex(messageIndex, object : CallbackListener<Message> {
                    override fun onSuccess(message: Message) {
                        // 6.x resolves temporary URLs for the whole attachment
                        // set at once; the pigeon API is single-media, so return
                        // the URL of the first attachment.
                        val media = message.attachedMedia.firstOrNull()
                            ?: return result.error(NotFoundException("No media attached to message: $messageIndex."))

                        message.getTemporaryContentUrlsForMedia(
                            listOf(media),
                            object : CallbackListener<Map<String, String>> {
                                override fun onSuccess(urls: Map<String, String>) {
                                    val url = urls[media.sid]
                                        ?: return result.error(NotFoundException("No temporary URL returned for media: ${media.sid}."))
